...
| `split_part` | `split_part(varchar, delimiter, index) → varchar` | rewrite | Rust UDF `trino_split_part`: 1-based; NULL past the last field (DataFusion returns `''`); `index < 1` is `INVALID_FUNCTION_ARGUMENT: Index must be greater than zero`, Trino's own wording in `StringFunctions.splitPart`; an empty delimiter splits into characters. |
...
<!-- GENERATED by glaux-fidelity (crates/glaux-fidelity) from a replay of the corpus.
...
Object keys are checked against Firehose's naming contract (prefix resolved at the flush instant, `<stream>-<version>-<yyyy-MM-dd-HH-mm-ss>-<uuid>[.ext]`) and snapshotted in template form; object contents are decoded (gunzipped, Parquet → Athena text) and pinned line by line.
