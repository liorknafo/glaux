-- CAST(... AS DECIMAL(p, s)): doubles through their exact binary expansion (HALF_UP), decimals rescaled HALF_UP, varchar parsed exactly (surrounding whitespace rejected, as by Java's BigDecimal), overflow loud.
