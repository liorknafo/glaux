...
pub static FUNCTIONS: &[FunctionShim] = &[
    ...
    shim!(
        ...
        "String",
        Rewrite,
        "Rust UDF `trino_split_part`: 1-based; NULL past the last field (DataFusion returns `''`); `index < 1` is `INVALID_FUNCTION_ARGUMENT: Index must be greater than zero`, Trino's own wording in `StringFunctions.splitPart`; an empty delimiter splits into characters.",
        ["trino_split_part"]
    ),
    shim!(
        "starts_with",
    ...
    shim!(
        ...
        "JSON",
        Udf,
        "Rust UDF: validates the text (invalid JSON is an error, like Trino) and keeps it as Trino's canonical text — the result column is reported to clients as `json` (see *Result metadata*): sorted keys, last duplicate key wins, integers exact at any size, other numbers in Java double text (`1e2` becomes `100.0`).",
        ["json_parse"]
    ),
    shim!(
        "json_size",
