...
pub static CONSTRUCTS: &[Construct] = &[
    ...
    Construct {
        ...
        category: "Semantics",
        status: ConstructStatus::Supported,
        notes: "Failures caused by the query's data (an invalid cast, an unparsable date, a bad subscript, an invalid regular expression, a missing regexp group) are user errors (Athena `ErrorCategory` 2) with Trino's error code; only I/O and engine failures are category 1. The codes follow Trino: integer overflow in a kernel is `NUMERIC_VALUE_OUT_OF_RANGE` (`abs(-9223372036854775808)`), `chr` outside the Unicode range and an unparsable `date_parse` input are `INVALID_FUNCTION_ARGUMENT`, and a scalar subquery returning several rows is `SUBQUERY_MULTIPLE_ROWS`. The messages name the reason alone: Arrow's and DataFusion's layer prefixes (`Arrow error: Compute error: `, `Execution error: `) and array-type names are stripped, and division by zero says `Division by zero` whether the operands are integers, doubles, or decimals. Integer negation overflow names the type and the value like Trino (`-CAST(-128 AS TINYINT)` is `tinyint negation overflow: -128`; Arrow said `Overflow happened on: - -128`). Integer casts carry Trino's wording rather than Arrow's: `CAST('1.5' AS INTEGER)` is `Cannot cast '1.5' to integer` and `CAST(2147483648 AS INTEGER)` is `Out of range for integer: 2147483648` (Arrow said `Cannot cast string '1.5' to value of Int32 type` and `Can't cast value ... to type Int32`).",
